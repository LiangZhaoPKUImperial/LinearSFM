    ...
    {	
        ...
        for ( i = 0; i < N; i++ )
		{
            ...
            else
			{
                ...
                double* ptrU, *ptrnU; // U \BA\CD newU ָ\D5\EB
                ...
                double* ptrW, *ptrnW, *ptrPID, *ptrV, *prtnV; // W, V \BA\CD newW ָ\D5\EB
...
bool CLinearSFMImp::run( int argc, char** argv )
{
	...
	if ( m_data == LinearSFM_Mono )
		CLinearSFMImp::runMono( m_szSt, m_szPose, m_szFeature, m_szPath, m_nMapCount );
	if ( m_data == LinearSFM_Stereo )
		CLinearSFMImp::runStereo( m_szSt, m_szPose, m_szFeature, m_szPath, m_nMapCount );

	return true;
}

bool CLinearSFMImp::lmj_parseArgs( int argc, char* argv[] )
{
	int i;
	string param;
	int  nFlag[5];
	...
	for (i = 1; i < argc; i++)
	{
		...
		if ( strcmp( name.c_str(), "f") == 0 )
		{
			i++;
			param = argv[i];
			m_szFeature = (char*)malloc(param.length());
			strcpy( m_szFeature, param.c_str() );
		}
		...
		if ( strcmp( name.c_str(), "type") == 0 )
		{
			i++;
			param = argv[i];
			if( strcmp(param.c_str(), "Monocular") == 0)
			{
				m_data = LinearSFM_Mono;
				nFlag[4] = 1;
			}

			if( strcmp(param.c_str(), "Stereo") == 0)
			{
				m_data = LinearSFM_Stereo;
				nFlag[4] = 1;
			}
		}
	}
	...
	if ( !nFlag[0] )
	{	printf( "LinerSFM Error: Please Input Right File Path:\n" );	return false;	}


	if ( !nFlag[2] )
	{	printf( "LinerSFM Error: Please Set Local Map Number:\n" );	return false;	}

	if ( !nFlag[4] )
	{	printf( "LinerSFM Error: Please Set Data Type:\n" );	return false;	}

	return true;

}

void CLinearSFMImp::lmj_printHelp()
{
	printf( "Linear SFM Solution General Options\n" );
	printf( "\n" );
	...
	printf( "-num			Number of Initial Recontruction\n");
